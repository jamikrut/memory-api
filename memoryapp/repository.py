...
def get_cards(category_id):
    categories_result = [category for category in categories_list if category.category_id == category_id]
...
def delete_card(category_id, card_id):
    ...
    cards_results = [card for card in cards_list if
                     card.category_id == category_id and card.card_id == card_id]

    if cards_results:
        cards_list[:] = [card for card in cards_list if
                         card.category_id != category_id or card.card_id != card_id]
    else:
